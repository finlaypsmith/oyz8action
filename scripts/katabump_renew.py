...
async def run():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                await page.locator('button[data-bs-target="#renew-modal"]').wait_for(
                    timeout=20000
                )
                log("✅ 页面加载完成")
            except:
            ...
            if await main_renew_btn.count() == 0:
                ...
                await page.screenshot(path=screenshot_path, full_page=True)
                tg_notify_photo(
                    screenshot_path, f"❌ 未找到 Renew 按钮\n服务器: {SERVER_ID}"
                )
            ...
            if await turnstile.count() > 0:
                ...
                try:
                    turnstile_iframe = page.frame_locator(
                        '#renew-modal iframe[src*="turnstile"]'
                    ).first
                    checkbox = turnstile_iframe.locator(
                        'input[type="checkbox"], .cb-i, #cf-stage'
                    )
                ...
                for i in range(30):
                    ...
                    if await response_input.count() > 0:
                        current_value = (
                            await response_input.get_attribute("value") or ""
                        )
                    ...
                    if i % 5 == 4:
                        ...
                        if i == 9:
                            screenshot_path = os.path.join(
                                SCREENSHOT_DIR, "turnstile_waiting.png"
                            )
                            await page.screenshot(path=screenshot_path, full_page=True)

                if not turnstile_token and CAPSOLVER_KEY:
                    turnstile_token = solve_turnstile_capsolver(
                        server_url, TURNSTILE_SITEKEY
                    )
                    if turnstile_token:
                        await page.evaluate(
                            "(token) => { document.querySelectorAll('input[name=\"cf-turnstile-response\"]').forEach(i => i.value = token); }",
                            turnstile_token,
                        )
                        log("✅ Token 已注入")
                ...
                if not turnstile_token:
                    log("❌ Turnstile 验证失败")
                    screenshot_path = os.path.join(
                        SCREENSHOT_DIR, "turnstile_failed.png"
                    )
                    await page.screenshot(path=screenshot_path, full_page=True)
            ...
            if await submit_btn.count() == 0:
                submit_btn = page.locator(
                    "#renew-modal .modal-footer button.btn-primary"
                )
            ...
            if "renew=success" in current_url:
                ...
                log("🔄 重新检查到期时间...")
                await page.goto(
                    server_url, timeout=60000, wait_until="domcontentloaded"
                )
...
def main():
    ...
    log(f"🌐 代理: {PROXY_SERVER if PROXY_SERVER else '未配置'}")
